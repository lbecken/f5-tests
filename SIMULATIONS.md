...
A human stick figure whose motion is generated by a real reduced-order model of
running rather than a hand-drawn animation loop.

**The figure.** Built to the classical **7.5-head canon** (1 unit = one head height),
with landmarks matching standard figure-drawing / anthropometric references: shoulders at
1.5 heads from the top, pelvis at 4.0, knee ~5.9, sole at 7.5. Drawn in the **sagittal
(side) plane** — the plane running physics live in — with near-side limbs bright and
far-side limbs dimmed for depth. A shared two-link **inverse-kinematics** solver places
knees and elbows from hip/shoulder and foot/hand targets.

**The physics: SLIP (Spring-Loaded Inverted Pendulum).** This is *the* canonical model
of running in biomechanics. The entire body is reduced to a point mass — the centre of
mass, taken at the pelvis — bouncing on a massless linear leg-spring:

- **Flight phase:** the COM is a pure projectile under gravity.
- **Stance phase:** one foot is planted and the leg is a spring of stiffness *k*, exerting
  `F = k(L0 − L)` along the line from foot to COM. The body compresses, stores elastic
  energy, and rebounds — exactly the "pogo-stick" behaviour real runners exhibit (tendons
  and muscles acting as springs).

Touchdown geometry uses **Raibert's foot-placement heuristic**: the foot is planted ahead
of the COM by an amount set by forward speed, which is what stabilises the running speed.
SLIP is energy-conserving, so it settles into a **steady periodic bounce** — the realistic
vertical oscillation of a runner *emerges from the physics*, not from a script. A light
regulator caps the vertical take-off speed (keeping the hop realistic, ~0.1 m) and nudges
horizontal speed toward the slider target, mirroring how real hopping controllers keep
hop height and forward speed independent. Integrated with fixed 1/240 s substeps.

**Hanging the skeleton on the physics.** The planted foot is the *real* SLIP contact
point and is IK-solved up to the hip, so **it never skates** — the classic giveaway of
faked running. The swing leg is driven as a **hip pendulum whose length shortens
mid-swing**: the knee flexes (heel toward buttock) and re-extends to reach the next
footfall, matching the measured swing-phase knee flexion (~30–60°) from gait analysis.
Arms counter-swing to the legs; the trunk leans forward with speed. HUD shows speed,
COM height, phase, cadence (steps/min) and distance. Sliders: target speed and leg
stiffness (stiffer legs → shorter, faster ground contact — a real, measurable effect).
...
- Running gait cycle joint angles (stance/swing hip–knee–ankle ranges): general gait-
  analysis literature — e.g. Fleet Feet's running gait cycle overview and PM&R
  KnowledgeNow, *Biomechanics of Normal Gait*.
- Countermovement-jump phase structure (unweighing → braking → propulsion → flight →
  landing) and landing force absorption: countermovement-jump biomechanics literature
  (e.g. Hawkin Dynamics, *Phases of the CMJ*; PMC, *Vertical Ground Reaction Forces in
  the Landing Phase of a Countermovement Jump*).
- The **SLIP** running model and **Raibert foot placement** are standard results in
  legged-locomotion / biomechanics (Blickhan 1989; Raibert, *Legged Robots That Balance*).
